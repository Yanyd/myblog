...
function drawCld(SY,SM) {
   ...
   if(SY>1949) yDisplay = '　人民共和国成立 ' + (((SY-1949)==1)?'元':SY-1949)
...
function mOvr(v) {
   ...
   if(sObj.innerHTML!='') {
      ...
      if(cld[d].solarTerms == '' && cld[d].solarFestival == '' && cld[d].lunarFestival == '')
         ...
         festival = '<TABLE WIDTH=100% BORDER=0 CELLPADDING=2 CELLSPACING=0 style="background-color:#CCFFCC;"><TR><TD>'+
         '<FONT COLOR="#000000" STYLE="font-size:9pt;">'+cld[d].solarTerms + ' ' + cld[d].solarFestival + ' ' + cld[d].lunarFestival+'</FONT></TD>'+
         '</TR></TABLE>';

      s= '<TABLE WIDTH="130" BORDER=0 CELLPADDING="2" CELLSPACING=0 style="background-color:#000066;"><TR><TD>' +
...
function tick() {
   var today
   today = new Date()
   $("#clock").html(today.toLocaleString().replace(/(年|月)/g, "/").replace(/日/, ""));
   $("#clock").html(TimeAdd(today.toGMTString(), CLD.TZ.value));
...
<SCRIPT language=JavaScript><!--
   ...
   document.write("<span style='color:red;'>你的浏览器无法执行此程序。此程序在 IE4 以后的版本才能执行!!</span>")
//--></SCRIPT>
...
<TABLE >
  <TBODY>
  <TR>
    <TD vAlign=top align=left ><BR><FONT style="FONT-SIZE: 16pt" 
      size=2>当前时间:</FONT><BR><FONT id="clock" face=Arial color=#000080 size=4 
      align="center"></FONT>
      ...
      <TABLE border=0 style="margin-left:70px;">
        ...
        <TR>
          <TD bgColor=#000080 colSpan=7><FONT style="FONT-SIZE: 9pt" 
            color=#ffffff size=2>　西 历　<SELECT style="FONT-SIZE: 9pt" 
   ...
   </TR></TBODY></TABLE></FORM></CENTER></BODY>
